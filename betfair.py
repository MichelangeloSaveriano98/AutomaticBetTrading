...
def parse_uo_button(row):
    ...
    bet_buttons = row.find('td', class_='coupon-runners').find_all('span')
    bets = [button.text if button.text[0] != '€' else button.text[1:] for button in bet_buttons]
    return {'u': SingleBetInfo(*bets[:4]),
            'o': SingleBetInfo(*bets[4:8])}
...
def parse_row(row, feature):
    ...
    if clubs:
        ...
        return (club1, club2, matchDate), feature_bets_parsers[feature](row)

def parse_content(page, sport, feature, driver_index, url, loading_period=2):
    driver = drivers[(sport, feature)][driver_index]
    ...
    data = {}
    for row in rows:
        key, value = parse_row(row, feature=feature)
        data[key] = value
...
class BetfairBettor(SiteBettor):
    def __init__(self, sport='calcio', feature='1x2', max_pages=10, n_drivers=-1):
        self.sport = sport
        self.feature = feature
        self.driver_id = (sport, feature)
    ...
    def set_feature(self):
        global drivers
        if self.feature != '1x2':
            is_uo = self.feature[:2] == 'uo'
            if is_uo:
                n_goal = self.feature[-3:]
            for driver in drivers[self.driver_id]:
    ...
    def reset_drivers(self, n_drivers=1):
        global drivers
        self.close()
        drivers[self.driver_id] = [webdriver.Chrome(executable_path='./chromeDriver/chromedriver.exe') for i in range(n_drivers)]
        self.setup_drivers()
