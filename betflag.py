...
def parse_12_button(row):
    ...
    if not (odds and len(odds) == 4 and liquidity and len(liquidity) == 6):
        return {'u': None, 'o': None}
    return {'u': SingleBetInfo(odds[0].text,
                               liquidity[1].text[:-1],
                               odds[2].text,
                               liquidity[4].text[:-1]),
            'o': SingleBetInfo(odds[1].text,
                               liquidity[2].text[:-1],
                               odds[3].text,
                               liquidity[5].text[:-1])}
...
class BetflagBettor(SiteBettor):
    def __init__(self, n_additional_data_loaded=4, sport='calcio', feature='1x2'):
    ...
    def get_data(self):
        ...
        for div in divs:
            key, value = parse_row(div, feature=self.feature)
            if value:
                data[key] = value
